...
linearmodel.save_weights('newRNNGATmodel_lstmlen_'+str(lstm_len)+'_hidden_unit_'+str(hh)+'_W_'+str(W)+'_F_'+str(F)+'.h5')

i=1
# name=np.arange(len(trainindex),len(trainindex)+len(trainindex))#np.arange(len(a[str(i)]['current'].keys()))
X_=np.zeros((len(testindex),16))
Y_=np.zeros((len(testindex),16))
X_org=np.zeros((len(testindex),16))
Y_org=np.zeros((len(testindex),16))

for idx,n in enumerate(testindex):
    X = {j:[] for j in range (2)}
    Y = {j:[] for j in range (2)}


    x = np.array(a[testindex[n][0]]['current'][str(testindex[n][1])])  # read dataset on the fly
    y = np.array(a[testindex[n][0]]['future'][str(testindex[n][1])]) # read dataset on the fly
    ...
    Y = [np.asarray(Y[j]) for j in Y]
    A=np.ones((X[0].shape[0],hh,hh))
    y_pred=linearmodel.predict([X,Y,A])
    

    Y_[idx,0*8:0*8+8]=y_pred[0][0][0,-1]
    Y_[idx,1*8:1*8+8]=y_pred[0][1][0,-1]
    

    # X_org[n,idx*14:idx*14+14]=X[idx][0,-1]
    Y_org[idx,0*8:0*8+8]=Y[0][0,-1]
    Y_org[idx,1*8:1*8+8]=Y[1][0,-1]
    
        # print(idx*2)
    # print(len(x_pred))
from sklearn.metrics import r2_scorepektralpektral
R=[]
for i in range (Y_.shape[-1]):
    R.append(r2_score(Y_org[:,i],Y_[:,i]))
    plt.figure(i+1)
    plt.plot(Y_[:,i]);
    plt.plot(Y_org[:,i]);
...
ax=sns.heatmap(data=np.array(R).reshape(2,-1),ax=ax,annot=True,vmin=0, vmax=0.5) 
fig.savefig('newR2_figure'+'_'+str(lstm_len)+'_hidden_unit_'+str(hh)+'_W_'+str(W)+'_F_'+str(F)+'.png')
